...
import { mockDailyReconciliation, BANK_ACCOUNTS, type DailyReconciliation } from "../data/mockDailyReconciliation";
...
function AmountCell({ value, type }: { value: number; type?: "in" | "out" }) {
  if (value === 0) return <span className="text-muted-foreground/40">—</span>;
  const color = type === "in" ? "text-emerald-600" : "text-red-500";
...
type AccountFilter = "All" | "Cash" | string;

const ACCOUNT_OPTIONS: AccountFilter[] = ["All", "Cash", ...BANK_ACCOUNTS];

// Aggregates bank columns across all selected accounts (or just one)
function getBankTotals(row: DailyReconciliation, filter: AccountFilter) {
  const accounts = filter === "All" || filter === "Cash"
    ? BANK_ACCOUNTS
    : [filter];

  return accounts.reduce(
    (acc, name) => {
      const b = row.banks[name];
      if (!b) return acc;
      return {
        opening: acc.opening + b.opening,
        in: acc.in + b.in,
        out: acc.out + b.out,
        closing: acc.closing + b.closing,
      };
    },
...
export default function DailyReconciliationTable() {
  ...
  const showCash = accountFilter === "All" || accountFilter === "Cash";
  const showBank = accountFilter === "All" || BANK_ACCOUNTS.includes(accountFilter);

  const bankLabel = accountFilter === "All"
    ? "Bank (All)"
    : BANK_ACCOUNTS.includes(accountFilter) ? accountFilter : "Bank";
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1.5">
          {ACCOUNT_OPTIONS.map((opt) => (
            <button
              key={opt}
              onClick={() => { setAccountFilter(opt); setCurrentPage(1); }}
          ...
          <table className="w-full text-sm text-left">
            <thead>
              ...
              <tr className="border-b border-border">
                <th className="px-5 py-3 text-muted-foreground font-medium text-[11px] uppercase tracking-wide whitespace-nowrap" />
                {showCash && (
                  <th
                    colSpan={4}
                    className="px-5 py-2.5 text-center text-amber-700 bg-amber-50/70 border-x border-amber-100 tracking-wide uppercase text-[10px] font-semibold"
                  >
                    Cash
                  </th>
                )}
                {showBank && (
                  <th
                    colSpan={4}
                    className="px-5 py-2.5 text-center text-blue-700 bg-blue-50/70 border-x border-blue-100 tracking-wide uppercase text-[10px] font-semibold"
                  >
                    {bankLabel}
                  </th>
                )}
                <th className="px-5 py-2.5 text-center text-muted-foreground text-[10px] uppercase tracking-wide font-medium" />
              </tr>
              {/* Sub-header */}
              <tr className="text-[11px] text-muted-foreground border-b border-border bg-muted/20">
                ...
                {showCash && <>
                  <th className="px-5 py-2.5 font-medium whitespace-nowrap text-left border-l border-amber-100 w-[110px]">Opening</th>
                  ...
                  <th className="px-5 py-2.5 font-medium whitespace-nowrap text-right border-r border-amber-100 w-[110px]">Closing</th>
                </>}
                {showBank && <>
                  <th className="px-5 py-2.5 font-medium whitespace-nowrap text-left border-l border-blue-100 w-[110px]">Opening</th>
                  ...
                  <th className="px-5 py-2.5 font-medium whitespace-nowrap text-right border-r border-blue-100 w-[110px]">Closing</th>
                </>}
                <th className="px-5 py-2.5 font-medium whitespace-nowrap text-center">Txns</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {paginated.map((row: DailyReconciliation) => {
                const bank = getBankTotals(row, accountFilter);
                return (
                  <tr key={row.date} className="hover:bg-muted/25 transition-colors group">
                    <td className="px-5 py-4 whitespace-nowrap">
                      <div className="font-semibold text-foreground text-sm">{format(parseISO(row.date), "dd MMM")}</div>
                      <div className="text-[11px] text-muted-foreground mt-0.5">{format(parseISO(row.date), "EEE")}</div>
                    </td>
